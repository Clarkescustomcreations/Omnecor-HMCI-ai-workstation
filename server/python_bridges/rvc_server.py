...
class RVCModel:
    ...
    def _load(self) -> None:
        ...
        # ── Real RVC integration: instantiate and populate net_g ──────────
        ...
        #     ["assets/hubert_base.pt"], suffix=""
        # )
        # self.hubert = models[0].to(self.device).eval()

        # STUB — remove when integrating the real model classes above.
        self.net_g  = checkpoint  # placeholder — stores the raw dict
        self.tgt_sr = OUTPUT_SAMPLE_RATE
        log.info("Checkpoint loaded (stub mode — swap in real model classes).")
    ...
    def convert(
        ...
        # (returns tensor of shape [1, T, 768])
        feats = self._stub_extract_features(audio_16k)          # ← REPLACE

        # ── Step 3: Pitch (F0) extraction ────────────────────────────────
        # f0, f0_coarse = self._extract_f0(audio_16k, pitch_shift)
        # (f0 in Hz, f0_coarse as semitone index)
        f0, f0_coarse = self._stub_extract_f0(audio_16k, pitch_shift)  # ← REPLACE

        # ── Step 4: Voice synthesis (VC forward pass) ─────────────────────
        # with torch.no_grad():
        #     audio_out = self.net_g.infer(
        #         feats, len(feats[0]), f0_coarse, f0,
        #         sid=torch.LongTensor([0]).to(self.device)
        #     )[0, 0]
        audio_out = self._stub_synthesise(feats, f0, pitch_shift)       # ← REPLACE
