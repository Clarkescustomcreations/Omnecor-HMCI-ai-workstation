...
export class ProcessManagerService extends EventEmitter {
  ...
  async spawnLoRATraining(config: {
    ...
    const { modelName, datasetPath, outputDir, epochs } = config;
  ...
  /**
   * Spawn a Blender headless script execution.
   *
   * @param scriptPath - Path to the Python script to execute inside Blender
   * @returns Job ID for tracking
   */
  async spawnBlenderJob(scriptPath: string): Promise<string> {
    await this.validatePath(scriptPath, "Blender script");

    return this.spawn({
      type: "blender",
      command: PYTHON_SCRIPTS.blenderBin,
      args: [
        "-b",                          // background/headless mode
        "-P", PYTHON_SCRIPTS.blenderBridge,  // our bridge script
        "--",                          // separator for custom args
        "--script", scriptPath,        // target script to execute
      ],
      label: `Blender: ${path.basename(scriptPath)}`,
      timeoutMs: 300000, // 5 minute timeout for Blender jobs
    });
  }

  /**
   * Spawn an ESP firmware flash operation.
   *
   * @param config - Flash configuration
   * @returns Job ID for tracking
   */
  async spawnESPFlash(config: {
    port: string;
    baud?: number;
    firmwarePath: string;
  }): Promise<string> {
    await this.validatePath(config.firmwarePath, "Firmware binary");
    ...
    return this.spawn({
      ...
      args: [
        ...
        "--port", config.port,
        "--baud", String(config.baud || 921600),
        "--firmware_path", config.firmwarePath,
      ],
      label: `ESP Flash: ${path.basename(config.firmwarePath)} → ${config.port}`,
