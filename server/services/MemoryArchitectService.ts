...
import { VectorDBService, DocumentInput, SearchResult } from "./VectorDBService";
...
export class MemoryArchitectService {
  ...
  async ensureProjectMemory(projectId: string): Promise<string> {
    ...
    await this.vectorDB.createCollection(name);
    return name;
  }
  ...
  async ingestDirectory(
    ...
    for (let i = 0; i < files.length; i += BATCH_SIZE) {
      ...
      if (documents.length > 0) {
        try {
          await this.vectorDB.generateAndStoreEmbeddings(collectionName, documents);
        } catch (error) {
          errors.push({
            file: `batch_${i}`,
  ...
  async ingestDocument(
    ...
    const documents: DocumentInput[] = chunks.map((chunk, idx) => ({
